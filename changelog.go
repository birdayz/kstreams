...
type StateRestoreListener interface {
	...
	OnRestoreStart(topicPartition TopicPartition, storeName string, startingOffset, endingOffset int64)
	...
	OnBatchRestored(topicPartition TopicPartition, storeName string, batchEndOffset int64, numRestored int64)

	// OnRestoreEnd is called when restoration completes
	// totalRestored: Total number of records restored
	OnRestoreEnd(topicPartition TopicPartition, storeName string, totalRestored int64)
}
...
func (n *NoOpRestoreListener) OnRestoreStart(topicPartition TopicPartition, storeName string, startingOffset, endingOffset int64) {
}

func (n *NoOpRestoreListener) OnBatchRestored(topicPartition TopicPartition, storeName string, batchEndOffset int64, numRestored int64) {
}

func (n *NoOpRestoreListener) OnRestoreEnd(topicPartition TopicPartition, storeName string, totalRestored int64) {
}
...
func (r *BatchingKeyValueStoreRestoreCallback[K, V]) restoreBatchOptimized(
	...
	for _, record := range records {
		...
		if record.Value == nil {
			...
			puts = append(puts, KV[K, V]{Key: k, Value: v})
		}
	}
