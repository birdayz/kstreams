...
type ProcessorNodeBatch[Kin, Vin, Kout, Vout any] struct {
	userProcessor    kprocessor.BatchProcessor[Kin, Vin, Kout, Vout]
...
func (p *ProcessorNodeBatch[Kin, Vin, Kout, Vout]) Process(ctx context.Context, k Kin, v Vin) error {
	...
	record := kprocessor.Record[Kin, Vin]{Key: k, Value: v}
	if err := p.userProcessor.ProcessBatch(ctx, []kprocessor.Record[Kin, Vin]{record}); err != nil {
		return err
	}
...
func (p *ProcessorNodeBatch[Kin, Vin, Kout, Vout]) ProcessBatch(ctx context.Context, keys []Kin, values []Vin) error {
	// Validate slice lengths match
	if len(keys) != len(values) {
		return fmt.Errorf("batch processing: keys and values slice length mismatch (keys=%d, values=%d)", len(keys), len(values))
	}
	...
	records := make([]kprocessor.Record[Kin, Vin], len(keys))
	for i := range keys {
		records[i] = kprocessor.Record[Kin, Vin]{
			Key:   keys[i],
			Value: values[i],
		}
	}
...
var _ kprocessor.BatchProcessorContext[string, string] = (*InternalBatchProcessorContext[string, string])(nil)

// ForwardBatch forwards multiple key-value pairs to all downstream processors.
func (c *InternalBatchProcessorContext[Kout, Vout]) ForwardBatch(ctx context.Context, records []kprocessor.KV[Kout, Vout]) error {
	if len(records) == 0 {
		return nil
	}
...
func (c *InternalBatchProcessorContext[Kout, Vout]) ForwardBatchTo(
	ctx context.Context,
	records []kprocessor.KV[Kout, Vout],
	childName string,
) error {
	if len(records) == 0 {
		return nil
	...
	if !ok {
		return fmt.Errorf("child processor %q not found", childName)
	}
