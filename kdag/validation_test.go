...
func newTestNode(id string, nodeType NodeType, children ...string) *Node {
...
func buildTestGraph(nodes ...*Node) *Graph {
	g := NewGraph()
...
func addTestSource(g *Graph, id, topic string, children ...string) {
...
func addTestStore(g *Graph, name string) {
...
func TestComputePartitionGroups(t *testing.T) {
	tests := []struct {
		...
		setup      func() *Graph
		wantErr    bool
		checkGroups func(*testing.T, []*coordination.PartitionGroup)
	}{
		{
			...
			wantErr: false,
			checkGroups: func(t *testing.T, groups []*coordination.PartitionGroup) {
