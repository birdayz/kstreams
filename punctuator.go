...
func (m *punctuationManager) schedule(
	...
	if pType == PunctuateByStreamTime {
...
// advanceStreamTime advances the stream time and triggers stream-time punctuators
func (m *punctuationManager) advanceStreamTime(ctx context.Context, timestamp time.Time) error {
	...
	return m.checkPunctuators(ctx, PunctuateByStreamTime, currentStreamTime)
}

// checkWallClock checks and triggers wall clock punctuators
func (m *punctuationManager) checkWallClock(ctx context.Context) error {
	...
	return m.checkPunctuators(ctx, PunctuateByWallClockTime, now)
}
